...
| ID | Issue |
|----|--------|
| — | [#48](https://github.com/lukejmorrison/canvasforge/issues/48) SIGABRT on quit during QGraphicsScene teardown |
...
- **Packaging:** Provide a platform-specific bundle (AppImage/Windows installer) so users can run CanvasForge without a dev environment. macOS Intel `.app` path exists via `scripts/build_macos_app.sh`. → [#24](https://github.com/lukejmorrison/canvasforge/issues/24) [#25](https://github.com/lukejmorrison/canvasforge/issues/25) [#26](https://github.com/lukejmorrison/canvasforge/issues/26)
