...
- **Fill tool + eyedropper:** Introduce a fill mode with a zoomed preview eyedropper, allowing users to sample on-canvas colors and fill selection cutouts or shapes. (Core fill/eyedropper + performance path landed; polish remains.)
...
- **Packaging:** Provide a platform-specific bundle (AppImage/Windows installer) so users can run CanvasForge without a dev environment. macOS Intel `.app` path exists via `scripts/build_macos_app.sh` (PyInstaller); still need AppImage/Windows and optional notarization.
