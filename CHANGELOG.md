...
### Fixed
- **macOS .app pairing:** The Intel PyInstaller spec now ships `mobile_sync`, the vendored QR encoder, and the LAN HTML fallback, so **Pair Mobile Device** works in `CanvasForge.app` built on a Mac Mini. The frozen app reads those files from the bundle extract dir.
